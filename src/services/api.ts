...
class ApiService {
  ...
  async createAppointment(data: ICreateAppointmentRequest): Promise<IAppointment> {
  ...
  // Procure pela classe ApiService e adicione este método se não existir:

  async deleteService(id: string): Promise<IApiResponse<IService>> {
    const response = await this.api.delete<IApiResponse<IService>>(`/services/${id}`);
    return response.data;
  }
